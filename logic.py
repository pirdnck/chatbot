...
st.markdown("<h1 class='main-title'>LIVESTOCKSHOP</h1>", unsafe_allow_html=True)
...
with col_meta:
    st.markdown("### Info Toko")
    with st.container(border=True):
        st.markdown("**🏡 Lokasi:** Sidodadi, Semarang")
        st.markdown("**⏱️ Jam Ambil:** 06:00 - 17:00 WIB")
        st.markdown("**🛡️ Jaminan:** 100% Organik & Segar")
    
    st.write("")
    st.markdown("### Mesin Sistem (FSM)")
    with st.container(border=True):
        st.markdown(
        ...
        with sub_cols[index % 2]:
            with st.container(border=True):
                ...
                if st.button(f"Ambil {key.capitalize()}", key=f"shop_{key}", use_container_width=True, disabled=is_disabled):
                    st.session_state.history.append({"role": "user", "content": f"Menambahkan 1 {key.capitalize()} via katalog"})
        ...
        if st.session_state.bot.cart:
            ...
            with st.container(border=True):
                ...
                for i, item in enumerate(st.session_state.bot.cart):
                    ...
                    with c_info:
                        st.markdown(f"<p class='item-title'>{item['item'].capitalize()}</p>", unsafe_allow_html=True)
                    ...
                    with c_sub:
                        ...
                        if st.button("Hapus", key=f"del_{item['item']}_{i}", use_container_width=True):
        ...
        if st.session_state.bot.invoice_logs:
            for nota in reversed(st.session_state.bot.invoice_logs):
                ...
                <div class='receipt-box'>
                    <strong>LIVESTOCKSHOP RECEIPT</strong><br>
                    --------------------------------<br>
                    No. Nota : {nota['id']}<br>
                    Waktu    : {nota['waktu']}<br>
                    --------------------------------<br>
