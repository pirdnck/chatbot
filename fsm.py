...
class LivestockFSM:
    def __init__(self):
        ...
        self.cart = []
        self.response = ""
        # Tempat penampungan riwayat nota lunas resmi di dalam mesin
        self.invoice_logs = []
        ...
        if self.state == State.IDLE:
            ...
            if intent == "ASK_MENU":
                ...
                if not self.cart:
                    ...
                    self.response = f"Total belanjaan hasil ternak: **Rp {self.calculate_total():,}**. Lanjut ke pembayaran? (Ketik **'Ya'** untuk konfirmasi / **'Tidak'** untuk batal)"
            else:
            ...
            # PROSES CETAK NOTA SEBELUM DATA DICLEAR
            id_nota = f"BT-{int(datetime.now().timestamp())}"
            waktu_cetak = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            
            nota_data = {
                "id": id_nota,
                "waktu": waktu_cetak,
                "items": list(self.cart), # Copy list keranjang
                "total": total
            }
            # Simpan nota secara permanen ke dalam list logs
            ...
            # Kembali ke IDLE & bersihkan keranjang aktif
            self.state = State.IDLE
            self.cart = []